...
def patient_register(request):
    if request.method == 'POST':
        ...
        postcode_area = request.POST.get('postcode_area', '')
        ...
        last = Patient.objects.order_by('-id').first()
        patient_id = f"PAT{(last.id + 1 if last else 1):05d}"
        ...
        messages.success(request, f'Registration successful! Your Patient ID is {patient.patient_id}. Please visit the clinic to book your appointment.')
        return render(request, 'registration/register.html')
